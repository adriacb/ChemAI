...
def forward_diffusion_old(data: dict, t: int, timesteps:int) -> dict:
    """
    ...
    #print(atom_types, noisy_atom_types)
    
    # Apply noise to bond types (Categorical noise)
    bond_types = data['edge_attr']
    eps = 1e-30
    num_classes = bond_types.size(-1)
    init_prob =  0.01 * np.ones(num_classes)                         # initialize with a small probability
    init_prob[0] = 1                                                 # absorv all states into the first one
    init_prob = init_prob / np.sum(init_prob)                        # normalize
    ...
    # add noise to bond types Vt = a * V0 + (1 - a) / K
    alphas = 1 - betas
    alpha_hat = torch.cumprod(alphas, dim=0)
    alpha_t = alpha_hat[t].reshape(1, -1)
    noise = torch.rand_like(bond_types)  # Use uniform noise for categorical data
    noisy_bond_types = torch.sqrt(alpha_t) * bond_types + torch.sqrt(1 - alpha_t) / num_classes
    print(bond_types, noisy_bond_types)
